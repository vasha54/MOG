/*
MatComGrader - E - Encoding Matrices
Tematica: Add-Hocc

Idea Basica: Ver cual simbolo se repite más y codificar según la regla
*/
import java.io.*;
import java.math.*;
import java.util.*;
...
public class Main {
    ...
    public void solve() throws Exception {
        ...
        char [][] encoded = new char [rows][rows];
        ...
        for(int i=0;i<rows;i++){
            ...
            for(int j=0;j<rows;j++){
                if(matrix[i].charAt(j)=='0') cZero++;
                if(matrix[i].charAt(j)=='1') cOne++;
            }
        }
        ...
        if(cZero > cOne || (cZero>=cOne && matrix[0].charAt(0)=='0')){
            one = 'o';
            zero = '*';
        }
        ...
        for(int i=0;i<rows;i++){
            for(int j=0;j<rows;j++){
                if(matrix[i].charAt(j)=='0') encoded[i][j]= zero;
                if(matrix[i].charAt(j)=='1') encoded[i][j]= one;
                out.printf("%c",encoded[i][j]);
            }
            out.printf("\n");
        }
    }
